...
from chatgpt_api import Chat, LLMAPIError, record_token_usage
...
import rich
import rich_utils
from project_paths import JARS_DIR, project_work_file
...
def ask_with_timeout(prompt, gpt4=False, timeout=90):
    ...
    process = pool.apply_async(Chat().sendMessagesWithUsage, args=(prompt, gpt4))
    pool.close()
    ...
    try:
        while True:
            ...
            if process.ready() == True:
                res = process.get()
                prompt_tokens = float(res.get("prompt_tokens", 0) or 0)
                completion_tokens = float(res.get("completion_tokens", 0) or 0)
...
def static_analysis(sink, source, file, function):
    dataflow_path = project_work_file(Path(file).resolve().parent, "dataflow.json")
    subprocess.run(
        [
            "java",
            "-jar",
            str(JARS_DIR / "SolidityStaticAnalysis-1.0-SNAPSHOT-standalone.jar"),
            file,
            function,
            str(dataflow_path),
        ]
    )
    data = json.load(open(dataflow_path, encoding="utf-8"))
...
def ask_for_static_json(prompts, source, answer_keys):
    ...
    # process answer
    answer = {}
    data = _extract_json_payload(res)
