...
whitelist = json.load(open(WHITELIST_PATH, encoding="utf-8"))
modifier_whitelist = json.load(open(MODIFIER_WHITELIST_PATH, encoding="utf-8"))
...
class CallGraph:
    def __init__(self, root:str) -> None:
        ...
        self.files = {}
        self.call_data = {}
        self.work_dir = ensure_project_work_dir(self.root)
        self.callgraph_file = self.work_dir / "callgraph.json"
    ...
    def __run_jar(self):
        subprocess.run(
            [
                "java",
                "-jar",
                str(JARS_DIR / "SolidityCallgraph-1.0-SNAPSHOT-standalone.jar"),
                self.root,
                str(self.callgraph_file),
            ],
            stdout=subprocess.DEVNULL,
            check=False,
        )
        self.call_data = json.load(open(self.callgraph_file, "r", encoding="utf-8"))
            ...
            for contract in self.call_data[file].copy():
                ...
                for contract_ in self.files[file]["subcontracts"]:
                    if contract_["name"] == contract:
                        match_contract_flag = True
                        matched_contract = contract_
                        break
                if match_contract_flag == False:
